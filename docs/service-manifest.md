...
مثلا فرض کنید سرویس شما می خواهد داده های مربوط به پایگاه داده خود را که در اینجا MySQL فرض شده است را از این به بعد در user's default volume ٬ بر روی آدرس جدیدی به نام var/lib/mysql/ ذخیره سازی کند برای همین sub_path مربوط به mysql از user's default volume را به آدرس جدید mount میکنیم.
