...
به ازای هر سرویس لازم است شما یک آبجکت بنویسید که فیلد‌های زیر را دارا باشد:
 ...
 نوع سرویسی که قصد دیپلوی آن را دارید، انواع سرویس این موارد هستند:
     ...
     - **InternalService** این مورد برای سرویس‌هایی که داخلی هستند، و فقط  توسط سرویس‌های دیگر شما استفاده می‌شوند، مثل دیتابیس‌ها، Message brokerها و...، قابل استفاده است.
     این نوع سرویس‌ها مثل سوئیچ `internal--` در دستور `fandogh service deploy‍` هستند.
     -  **ManagedService** از این مورد برای دیپلوی سرویس‌های مدیریت شده فندق مثل mysql یا Postgresql باید استفاده کنید.
 - **name**
...
> چند نکته مهم در رابطه با استفاده از command
> - اگر از دستور command و command_args استفاده نکنید٬ دستور پیش فرض موجود در Docker Image اجرا خواهد شد.
> - اگر از دستور command استفاده کنید ولی command_args را وارد نکنید٬ command اجرا خواهد شد و CMD و ENTRYPOINT تعریف شده در Docker Image نادیده گرفته خواهند شد.
