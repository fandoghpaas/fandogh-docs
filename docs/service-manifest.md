...
مقدار پیشفرض برای پروتکل TCP است، پس نیازی نیست همیشه قید کنید اگر به UDP نیازی ندارید.
 ...
 - **volume_mounts**
volume_mounts یک لیست از آدرس های محل ذخیره سازی می باشد که به volume اصلی mount می شوند و هر container می تواند دیتاهای خود را در محلی که در manifest قرار داده شده است نوشته و ذخیره کند.
